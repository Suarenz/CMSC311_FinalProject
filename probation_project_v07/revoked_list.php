                                ...
                                <div class="row mb-4">
                                    ...
                                    <div class="col-md-2">
                                        <form action="export_pdf.php" method="post">
                                            <button type="submit" name="revoked_pdf" class="btn btn-danger form-control">
                                    ...
                                    <table class="table table-hover table-bordered">
                                        <thead class="table-dark">
                                            <tr>
                                                ...
                                                <th scope="col" class="text-nowrap text-center">Name<a href="revoked_list.php?sort=name"><i class="mdi mdi-sort-alphabetical"></i></a></th>
                                                <th scope="col" class="text-nowrap text-center">Alias<a href="revoked_list.php?sort=alias"><i class="mdi mdi-sort"></i></a></th>
                                                <th scope="col" class="text-nowrap text-center">Municipality</th>
                                                <!-- <th scope="col">Case No.</th> -->
                                                <th scope="col" class="text-nowrap text-center">Status</th>
                                                <th scope="col" class="text-nowrap text-center">Date Revoked<a href="revoked_list.php?sort=dr"><i class="mdi mdi-sort"></i></a></th>
                                        ...
                                        <tbody>
                                            ...
                                            $query = "SELECT c.*, DATE_FORMAT(rc.date_revoked, '%b. %d, %Y') AS dr FROM clients c JOIN revoked_clients rc ON c.id = rc.client_id WHERE c.status = 'revoked' ORDER BY $order_by LIMIT $start, $display";
                                ...
                                <nav aria-label="Page navigation">
                                    <ul class="pagination">
                                        ...
                                        if ($pages > 1) :
                                            ...
                                            if ($current_page != 1) :
                                        ?>
                                                <li class="page-item"><a class="page-link" href="revoked_list.php?s=<?= ($start - $display) ?>&p=<?= $pages ?>&sort=<?= $sort ?>">Previous</a></li>
                                            <?php endif; ?>

                                            <!-- Make all the numbered pages: -->
                                            <?php for ($i = 1; $i <= $pages; $i++) : ?>
                                                <li class="page-item <?= ($i == $current_page) ? 'active" aria-current="page">' : '">' ?>
                                                    <a class=" page-link" href="revoked_list.php?s=<?= (($display * ($i - 1))) ?>&p=<?= $pages ?>&sort=<?= $sort ?>"><?= $i ?></a>
                                                </li>
                                            <?php endfor; ?>

                                            <!-- If it's not the last page, make a Next button: -->
                                            <?php if ($current_page != $pages) : ?>
                                                <li class="page-item"><a class="page-link" href="revoked_list.php?s=<?= ($start + $display) ?>&p=<?= $pages ?>&sort=<?= $sort ?>">Next</a></li>
                                            <?php endif; ?>
                                        <?php endif; ?>
                                    </ul>
                                </nav>
